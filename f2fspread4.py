...
instruments_mtime = None
stocks_mtime = None
cached_instrument_data = None
cached_underlyings = None
market_state = {}
...
def check_file_updates():
    global instruments_mtime, stocks_mtime, cached_instrument_data, cached_underlyings
    current_instruments_mtime = os.path.getmtime(INSTRUMENTS_JSON) if os.path.exists(INSTRUMENTS_JSON) else 0
    current_stocks_mtime = os.path.getmtime(STOCKS_CSV) if os.path.exists(STOCKS_CSV) else 0

    if cached_instrument_data is None or instruments_mtime != current_instruments_mtime:
        cached_instrument_data = load_instruments()
        instruments_mtime = current_instruments_mtime
        print(f"[{datetime.now()}] Loaded instruments.json")

    if cached_underlyings is None or stocks_mtime != current_stocks_mtime:
        cached_underlyings = load_underlyings()
        stocks_mtime = current_stocks_mtime
        print(f"[{datetime.now()}] Loaded futurestockslist.csv")

    return cached_instrument_data, cached_underlyings
...
instrument_data, underlyings = check_file_updates()
subscribe_keys = []
...
def start_websocket_server():
        ...
        while True:
            if update_queue:
                with market_state_lock:
                    ...
                    # Rebuild only for affected underlying
                    for sym in cached_underlyings:
                        futs = get_instrument_keys_for_symbol(sym, cached_instrument_data)
                        if any(f["instrument_key"] == affected_symbol for f in futs):
                            df = build_df([sym], cached_instrument_data)
    ...
    socketio.run(flask_app, host="0.0.0.0", port=WEBSOCKET_PORT)
